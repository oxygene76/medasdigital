...
install: enforce-go-version
	@echo "🤖 Installing medasdigitald..."
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/medasdigitald
	@echo "✅ Completed installation!"

build: enforce-go-version
	@echo "🤖 Building medasdigitald..."
	go build $(BUILD_FLAGS) -o $(BUILDDIR)/ ./cmd/medasdigitald
...
build-reproducible-amd64: go.sum $(BUILDDIR)/
	$(DOCKER) buildx create --name medasdigitalbuilder || true
	$(DOCKER) buildx use medasdigitalbuilder
	$(DOCKER) buildx build \
		...
		-t medasdigital:local-amd64 \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f medasdigitalbinary || true
	$(DOCKER) create -ti --name medasdigitalbinary medasdigital:local-amd64
	$(DOCKER) cp medasdigitalbinary:/bin/medasdigitald $(BUILDDIR)/medasdigitald-linux-amd64
	$(DOCKER) rm -f medasdigitalbinary

build-reproducible-arm64: go.sum $(BUILDDIR)/
	$(DOCKER) buildx create --name medasddigitalbuilder || true
	$(DOCKER) buildx use medasdigitalbuilder
	$(DOCKER) buildx build \
		--build-arg GO_VERSION=$(GO_VERSION) \
		--build-arg GIT_VERSION=$(VERSION) \
		...
		-t medasdigital:local-arm64 \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f medasdigitalbinary || true
	$(DOCKER) create -ti --name medasdigitalbinary medasdigital:local-arm64
	$(DOCKER) cp medasdigitalbinary:/bin/medasdigitald $(BUILDDIR)/medasdigitald-linux-arm64
	$(DOCKER) rm -f medasdigitalbinary
...
containerProtoGenGo=medasdigital-proto-gen-go-$(protoVer)
containerProtoGenSwagger=medasdigital-proto-gen-swagger-$(protoVer)
...
docker-build:
	@DOCKER_BUILDKIT=1 $(DOCKER) build \
		-t medasdigital:local \
		-t medasdigital:local-distroless \
